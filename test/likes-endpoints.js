...
describe("Likes Endpoints", function () {
  ...
  before("clean the table", () =>
    db.raw("TRUNCATE likes, videos RESTART IDENTITY CASCADE")
  );
  ...
  describe(`GET /likes`, () => {
    context("Given there are no likes in the database", () => {
      it(`responds with 200 and an empty list`, () => {
        return supertest(app).get("/api/likes").expect(200, []);
      });
    });
    context("Given there are likes in the database", () => {
      ...
      beforeEach("insert likes", () => {
        return db
          ...
          .insert(testVideos)
          .then(() => {
            return db.into("likes").insert(testLikes);
          });
      });
      it("GET /comments responds with 200 and all of the likes", () => {
        return supertest(app).get("/api/likes").expect(200, testLikes);
      });
    });
  });
});
